...
func (s *AntrianService) GetDetailAntrianByID(idAntrian int) (map[string]interface{}, error) {
	// --------------------------------------------------------------------
	// 1. Ambil data dasar pasien + antrian + rekam medis (seperti sebelumnya)
	// --------------------------------------------------------------------
	queryDetails := `
		SELECT p.id_pasien, p.nama, p.jenis_kelamin, p.tempat_lahir,
		       DATE_FORMAT(p.tanggal_lahir,'%Y-%m-%d') AS tanggal_lahir,
		       p.nik, p.no_telp, p.alamat, p.kota_tinggal,
	...
	var (
		...
		nama, jk, tempatLahir, tglLahirStr      string
		nik, noTelp, alamat, kota, kel, kec     string
		keluhanUtama, idRM                      string
	)
	if err := s.DB.QueryRow(queryDetails, idAntrian).Scan(
		&idPasien, &nama, &jk, &tempatLahir, &tglLahirStr,
		&nik, &noTelp, &alamat, &kota, &kel, &kec,
	...
	// --------------------------------------------------------------------
	// 2. Cari id_assessment terkini (jika ada) dari Riwayat_Kunjungan
	// --------------------------------------------------------------------
	...
	if err := s.DB.QueryRow(`
		...
		LIMIT  1
	`, idAntrian).Scan(&idAssessment); err != nil && err != sql.ErrNoRows {
		return nil, fmt.Errorf("failed to get assessment id: %v", err)
	}
	...
	umur := time.Now().Year() - tglLahir.Year()
	if time.Now().YearDay() < tglLahir.YearDay() {
		umur--
	}
	...
	res := map[string]interface{}{
		...
		"id_assessment":  nil,           // default null
	}
	if idAssessment.Valid {
		res["id_assessment"] = int(idAssessment.Int64)
	}
